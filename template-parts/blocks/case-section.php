<?php
/**
 * Блок «Кейсы»: заголовок, текст, фильтр case_cat, слайдер карточек.
 * Фильтр — REST /tolstenko/v1/filter-posts, карточки через case-card.php.
 */
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

$block_attrs = get_query_var( 'tolstenko_block_attributes', array() );
if ( ! is_array( $block_attrs ) ) {
	$block_attrs = array();
}

$defaults = function_exists( 'tolstenko_get_block_defaults' ) ? tolstenko_get_block_defaults( 'case_section' ) : array();
if ( ! is_array( $defaults ) ) {
	$defaults = array();
}

$title = '';
if ( ! empty( $block_attrs['block_case_section_title'] ) ) {
	$title = (string) $block_attrs['block_case_section_title'];
} elseif ( ! empty( $defaults['title'] ) ) {
	$title = (string) $defaults['title'];
}

$text = '';
if ( isset( $block_attrs['block_case_section_text'] ) && trim( (string) $block_attrs['block_case_section_text'] ) !== '' ) {
	$text = (string) $block_attrs['block_case_section_text'];
} elseif ( ! empty( $defaults['text'] ) ) {
	$text = (string) $defaults['text'];
}

$title_tag = function_exists( 'tolstenko_normalize_heading_tag' )
	? tolstenko_normalize_heading_tag( $block_attrs['block_case_section_title_tag'] ?? 'h2', 'h2' )
	: 'h2';

$posts_per_page = isset( $block_attrs['block_case_section_posts_per_page'] )
	? (int) $block_attrs['block_case_section_posts_per_page']
	: ( isset( $defaults['posts_per_page'] ) ? (int) $defaults['posts_per_page'] : 4 );
if ( $posts_per_page === 0 ) {
	$posts_per_page = 4;
}

$section_id = 'cases_' . wp_unique_id();
$taxonomy   = 'case_cat';
$post_type  = 'case';
$card       = 'case';

$categories_with_posts = function_exists( 'tolstenko_get_filter_top_level_terms' )
	? tolstenko_get_filter_top_level_terms( $taxonomy )
	: array();

$active_term = function_exists( 'tolstenko_get_filter_active_term_slug' )
	? tolstenko_get_filter_active_term_slug( $taxonomy )
	: '';
if ( function_exists( 'tolstenko_ensure_filter_term_in_categories' ) ) {
	$categories_with_posts = tolstenko_ensure_filter_term_in_categories( $categories_with_posts, $taxonomy, $active_term );
}

$items_html = function_exists( 'tolstenko_render_filtered_posts_html' )
	? tolstenko_render_filtered_posts_html(
		array(
			'post_type'      => $post_type,
			'taxonomy'       => $taxonomy,
			'term'           => $active_term,
			'posts_per_page' => $posts_per_page,
			'card'           => $card,
		)
	)
	: '';

if ( $title === '' && $text === '' && $items_html === '' && empty( $categories_with_posts ) ) {
	return;
}
?>
<section
	class="case-section section"
	data-tolstenko-filter
	data-section-id="<?php echo esc_attr( $section_id ); ?>"
	data-taxonomy="<?php echo esc_attr( $taxonomy ); ?>"
	data-post-type="<?php echo esc_attr( $post_type ); ?>"
	data-posts-per-page="<?php echo esc_attr( (string) $posts_per_page ); ?>"
	data-card="<?php echo esc_attr( $card ); ?>"
>
	<div class="container">
		<div class="case-section__top section-top">
			<?php if ( $title !== '' ) : ?>
				<<?php echo esc_attr( $title_tag ); ?> class="case-section__title h2"><?php echo tolstenko_kses_html( $title ); ?></<?php echo esc_attr( $title_tag ); ?>>
			<?php endif; ?>
			<?php if ( $text !== '' ) : ?>
				<p class="case-section__text paragraph-15-15"><?php echo tolstenko_kses_html( $text ); ?></p>
			<?php endif; ?>
		</div>

		<?php if ( ! empty( $categories_with_posts ) ) : ?>
			<div class="case-section__filter filter filter--slider">
				<div class="filter__form swiper">
					<div class="swiper-wrapper">
						<label class="filter__radio swiper-slide">
							<input
								type="radio"
								name="<?php echo esc_attr( $section_id ); ?>_category"
								value=""
								data-section-id="<?php echo esc_attr( $section_id ); ?>"
								class="tolstenko-filter-radio"
								<?php checked( $active_term, '' ); ?>
							>
							<span class="filter__label"><?php esc_html_e( 'Все кейсы', 'tolstenko-theme' ); ?></span>
						</label>
						<?php foreach ( $categories_with_posts as $cat ) : ?>
							<label class="filter__radio swiper-slide">
								<input
									type="radio"
									name="<?php echo esc_attr( $section_id ); ?>_category"
									value="<?php echo esc_attr( $cat->slug ); ?>"
									data-section-id="<?php echo esc_attr( $section_id ); ?>"
									class="tolstenko-filter-radio"
									<?php checked( $active_term, $cat->slug ); ?>
								>
								<span class="filter__label"><?php echo esc_html( $cat->name ); ?></span>
							</label>
						<?php endforeach; ?>
					</div>
				</div>
				<?php get_template_part( 'template-parts/filter-arrows' ); ?>
			</div>
		<?php endif; ?>

		<?php if ( $items_html !== '' ) : ?>
			<div
				class="fade-in-container case-section__splide splide"
				id="<?php echo esc_attr( $section_id ); ?>-container"
				aria-label="<?php esc_attr_e( 'Кейсы', 'tolstenko-theme' ); ?>"
			>
				<div class="splide__track swiper">
					<div
						class="case-section__splide-list splide__list swiper-wrapper"
						data-tolstenko-filter-container
					>
						<?php echo $items_html; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- built from escaped card template. ?>
					</div>
				</div>

				<div class="splide__bottom">
					<div class="swiper-pagination splide__pagination"></div>

					<div class="splide__arrows splide__arrows--ltr">
						<button class="splide__arrow splide__arrow--prev" type="button" aria-label="<?php esc_attr_e( 'Назад', 'tolstenko-theme' ); ?>">
							<svg viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
								<path d="M15.8332 10H4.99987M9.16654 5L4.7558 9.41074C4.43036 9.73618 4.43036 10.2638 4.7558 10.5893L9.16654 15" stroke-width="2" stroke-linecap="round" />
							</svg>
						</button>

						<button class="splide__arrow splide__arrow--next" type="button" aria-label="<?php esc_attr_e( 'Вперёд', 'tolstenko-theme' ); ?>">
							<svg viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
								<path d="M15.8332 10H4.99987M9.16654 5L4.7558 9.41074C4.43036 9.73618 4.43036 10.2638 4.7558 10.5893L9.16654 15" stroke-width="2" stroke-linecap="round" />
							</svg>
						</button>
					</div>
				</div>
			</div>
		<?php endif; ?>
	</div>
</section>
<?php
set_query_var( 'tolstenko_case_post', null );
set_query_var( 'tolstenko_case_card_class', '' );
?>

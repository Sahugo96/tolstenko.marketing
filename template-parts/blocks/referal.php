<?php
/**
 * Блок «Рефералка» (партнёрская секция referal).
...
$defaults = function_exists( 'tolstenko_get_block_defaults' ) ? tolstenko_get_block_defaults( 'referal' ) : array();
if ( ! is_array( $defaults ) ) {
	$defaults = array();
}

$title = ! empty( $block_attrs['block_referal_title'] )
	? (string) $block_attrs['block_referal_title']
	: (string) ( $defaults['title'] ?? '' );
$title_tag = function_exists( 'tolstenko_normalize_heading_tag' )
	? tolstenko_normalize_heading_tag( $block_attrs['block_referal_title_tag'] ?? 'h2', 'h2' )
	: 'h2';

$items     = array();
$raw_items = ! empty( $block_attrs['block_referal_items'] ) && is_array( $block_attrs['block_referal_items'] )
	? $block_attrs['block_referal_items']
...
$list_title = ! empty( $block_attrs['block_referal_list_title'] )
	? (string) $block_attrs['block_referal_list_title']
	: (string) ( $defaults['list_title'] ?? '' );

$list     = array();
$raw_list = ! empty( $block_attrs['block_referal_list'] ) && is_array( $block_attrs['block_referal_list'] )
	? $block_attrs['block_referal_list']
...
$btn_text = ! empty( $block_attrs['block_referal_btn_text'] )
	? (string) $block_attrs['block_referal_btn_text']
	: (string) ( $defaults['btn_text'] ?? '' );
$btn_url = ! empty( $block_attrs['block_referal_btn_url'] )
	? (string) $block_attrs['block_referal_btn_url']
...
if ( $title === '' && empty( $items ) && empty( $list ) && $btn_text === '' ) {
	return;
}
?>
<section class="referal section">
	<div class="container">
		<div class="referal__inner br-30">
			<div class="section-top">
				<?php if ( $title !== '' ) : ?>
					<<?php echo esc_attr( $title_tag ); ?> class="referal__title h2"><?php echo tolstenko_kses_html( $title ); ?></<?php echo esc_attr( $title_tag ); ?>>
				<?php endif; ?>
			</div>
				...
				<?php if ( ! empty( $items ) ) : ?>
					<div class="referal__items">
						<?php foreach ( $items as $item_text ) : ?>
							<div class="referal__item border-card">
								<p class="referal__item-text line-13-15"><?php echo tolstenko_kses_html( $item_text ); ?></p>
							</div>
						<?php endforeach; ?>
					</div>
				<?php endif; ?>

				<div class="referal__right">
					<?php if ( $list_title !== '' || ! empty( $list ) ) : ?>
						<div class="referal__right-wrapper">
							<?php if ( $list_title !== '' ) : ?>
								<span class="referal__right-title"><?php echo tolstenko_kses_html( $list_title ); ?></span>
							<?php endif; ?>

							<?php if ( ! empty( $list ) ) : ?>
								<div class="referal__list">
									<?php foreach ( $list as $list_text ) : ?>
										<div class="referal__list-item border-card">
											...
											<p class="referal__list-text line-13-15"><?php echo tolstenko_kses_html( $list_text ); ?></p>
										</div>
									<?php endforeach; ?>
								</div>
							<?php endif; ?>
					...
					<?php if ( $btn_text !== '' ) : ?>
						<a
							class="referal__btn default-btn line-caps-bold-13-15"

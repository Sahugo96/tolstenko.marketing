...
$defaults = tolstenko_block_defaults( 'recomendation' );

$title = ! empty( $block_attrs['block_recomendation_title'] )
	? (string) $block_attrs['block_recomendation_title']
	: (string) ( $defaults['title'] ?? '' );
$title_tag = tolstenko_block_heading_tag( $block_attrs, 'block_recomendation_title_tag', 'h2' );
...
$render_ico = static function ( $attachment_id ) {
	...
	if ( $path && is_readable( $path ) && preg_match( '/\.svg$/i', $path ) ) {
		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents -- theme SVG from media library.
		$svg = file_get_contents( $path );
		if ( $svg ) {
			echo $svg; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- trusted SVG from media.
			return;
		}
	}

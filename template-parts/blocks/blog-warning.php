 ...
 *
 * Типы:
 * - warn / pin / ide — встроенные иконки + цветной фон (без изменений).
 * - custom — своя иконка (необязательна). Без иконки: тёмная плашка, светлый текст по центру.
 *   Нельзя подставлять дефолтный .warn — иначе ломается вид «кастомный без иконки».
 */
if ( ! defined( 'ABSPATH' ) ) {
	exit;
}
...
/**
 * HTML иконки или пустая строка (для custom без/с битой иконкой — без fallback на .warn).
 *
	...
	if ( $type === 'custom' ) {
		...
		if ( $path && is_readable( $path ) && preg_match( '/\.svg$/i', $path ) ) {
			...
			if ( $svg !== '' && stripos( $svg, '<svg' ) !== false ) {
				// Помечаем корневой svg классом custom (если class ещё нет).
...
<div class="single-blog__content-block single-blog__content-block--warning br-30">
	<ul class="single-blog__warnings">
		<?php foreach ( $clean as $item ) : ?>
			...
			$classes   = array( 'single-blog__warnings-item' );

			if ( $type === 'custom' ) {
				$classes[] = 'single-blog__warnings-item--custom';
				if ( $icon_html === '' ) {

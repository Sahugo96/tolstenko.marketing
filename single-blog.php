...
<main class="main">
	...
	while ( have_posts() ) {
		...
		// Слайдер услуг (без фильтра): поля статьи → иначе дефолты «Слайдер услуг».
		...
		// Похожие статьи: поля статьи → иначе дефолты «Похожие статьи».
		...
		// FAQ: поля статьи → иначе дефолты «Настройки сайта → FAQ».
		...
		// Стандартный сквозной блок (дефолты: Настройки сайта → Бесплатная консультация).
